...
def note_form_html(form):
    ...
    <form hx-post="{url}">
     {form}
     <input type="submit">
    </form>''', url=reverse(create_note_hxpost), form=form)
...
def note_and_next_html(note):
    ...
    if next:
        next_html = format_html('<div hx-get="{}" hx-trigger="revealed" hx-swap="outerHTML">...</div>',
                              reverse('note_and_next_hx', kwargs=dict(note_id=next.id)))
    else:
        next_html = 'The End'
    return format_html('{note_html} {next_html}',
